export class ModalManager {
    ...
    setButtonContent(readBtn, sideBtns, actionText, url) {
        ...
        readBtn.innerHTML = `<a href="${url}" target="_blank" rel="noreferrer" class="d-flex align-items-center justify-content-center flex-column gap-2 p-3 text-center"><i class="fa-solid fa-file-arrow-down"></i>${actionText}</a><button id="addToCalendarBtnMobile" type="button" class="d-flex d-lg-none align-items-center justify-content-center flex-column gap-2 p-3 text-center"><i class="fa-regular fa-calendar-plus"></i><span>Ajouter à mon calendrier</span></button>`;
    ...
    setupCalendarButtons(event) {
        ...
        //GOOGLE
        document.getElementById('googleCalendar').addEventListener('click', () => this.googleCalendar(event)); 

        //OUTLOOK
        document.getElementById('outlookCalendar').addEventListener('click', () => this.outlookCalendar(event));

        //APPLE
        document.getElementById('appleCalendar').addEventListener('click', ()=> this.appleCalendar(event));
